...
class TestSQL:
    def test_read_sql(self) -> None:
        ...
        cur = Cursor()
        t = Tafra.read_sql(
            'SELECT * FROM [Table]', cur)  # type: ignore
        check_tafra(t)
    ...
    def test_invalid_assignment(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert str(w[0].message) == (
                '`np.squeeze(ndarray)` applied to set ndim == 1.')
        ...
        with pytest.raises(Exception) as e:
            t['x'] = np.repeat(
                np.arange(6)[:, None], repeats=2, axis=1)
...
class TestFormatting:
    ...
    def test_object_formatter_validation(self) -> None:
        ...
        with pytest.raises(ValueError):
            object_formatter['Bad'] = (
                lambda x: 'not an array')  # type: ignore

        with pytest.raises(ValueError):
            object_formatter['Bad'] = (
                lambda x: x.no_such_method())  # type: ignore

    def test_object_parse(self) -> None:
        t = build_tafra()
        t['d'] = np.array([
            datetime.fromisoformat(f'2020-0{_+1}-01')
            for _ in range(6)
        ])
        assert t._dtypes['d'] == 'object'
        check_tafra(t)

        object_formatter['datetime'] = (
            lambda x: x.astype('datetime64[D]'))
...
class TestIteration:
    ...
    def test_iterate_by_attr(self) -> None:
        ...
        for i, (u, ix, grouped) in enumerate(
            t.iterate_by(['y', 'z'])
        ):
...
class TestGroupBy:
    ...
    def test_build_group_indices_single_col(self) -> None:
        ...
        np.testing.assert_array_equal(
            indices[0], np.array([0, 2]))
        np.testing.assert_array_equal(
            indices[1], np.array([1, 3]))
        np.testing.assert_array_equal(
            indices[2], np.array([4]))
    ...
    def test_build_group_indices_multi_col(self) -> None:
        ...
        unique, indices = GroupSet._build_group_indices(
            t, ['a', 'b'])
        assert unique[0] == (1, 'x')
        assert unique[1] == (1, 'y')
        assert unique[2] == (2, 'x')
        assert unique[3] == (2, 'y')
        np.testing.assert_array_equal(
            indices[0], np.array([0, 4]))
        np.testing.assert_array_equal(
            indices[1], np.array([1]))
        np.testing.assert_array_equal(
            indices[2], np.array([2]))
        np.testing.assert_array_equal(
            indices[3], np.array([3]))
    ...
    def test_vectorized_std_var(self) -> None:
        ...
        gb = t.group_by(
            ['g'], {'s': (np.std, 'v'), 'va': (np.var, 'v')})
        np.testing.assert_almost_equal(
            gb['s'][0], np.std([10., 20., 30.]))
        np.testing.assert_almost_equal(
            gb['va'][0], np.var([10., 20., 30.]))
        np.testing.assert_almost_equal(
            gb['s'][1], np.std([40., 50.]))
    ...
    def test_percentile_agg(self) -> None:
        ...
        np.testing.assert_almost_equal(
            gb['p90'][0],
            np.percentile([10., 20., 30., 40., 50.], 90),
        )
    ...
    def test_geomean_harmean(self) -> None:
        ...
        gb = t.group_by(
            ['g'], {'geo': (geomean, 'v'), 'har': (harmean, 'v')})
        np.testing.assert_almost_equal(
            gb['geo'][0], (2. * 4. * 8.) ** (1./3.))
        np.testing.assert_almost_equal(
            gb['har'][0], 3. / (1./2. + 1./4. + 1./8.))
...
class TestTransform:
    ...
    def test_transform_values_match_original(self) -> None:
        ...
        np.testing.assert_array_almost_equal(
            tr['mean_val'][:2], [15.0, 15.0])
        np.testing.assert_array_almost_equal(
            tr['mean_val'][2:], [40.0, 40.0, 40.0])
...
class TestDatetime:
    def test_datetime(self) -> None:
        t = build_tafra()
        t['d'] = np.array(
            [np.datetime64(_, 'D') for _ in range(6)])
...
class TestCSV:
    def test_csv(self) -> None:
        ...
        t.update_dtypes_inplace({
            'dp_prime': float, 'dp_prime_te': 'float64'})
        assert t.dtypes['dp_prime'] == 'float64'
        assert t.dtypes['dp_prime_te'] == 'float64'
        check_tafra(t)
...
class TestStringDtype:
    ...
    def test_join_different_u_widths(self) -> None:
        ...
        right = Tafra({
            'k': np.array(
                ['DELAWARE BASIN', 'MIDLAND'], dtype='<U14'),
            'w': np.array([10, 20]),
        })
        t = left.left_join(right, on=[('k', 'k', '==')])
        assert len(t) == 2
    ...
    def test_left_join_warns_on_object_fallback(self) -> None:
        ...
        msgs = [x for x in w
                if 'cast to object' in str(x.message)]
        assert len(msgs) == 1
    ...
    def test_left_join_object_fallback_warning(self) -> None:
        ...
        object_warnings = [
            x for x in w if 'cast to object' in str(x.message)]
        ...
        assert t2['name'].dtype == np.dtypes.StringDType(
            na_object=None)
...
class TestLeftJoin:
    ...
    def test_left_join_exact_pairs(self) -> None:
        ...
        assert t['rv'][idx2] == 200.
        assert t['rv'][idx3] == 300.
    ...
    def test_left_join_null_fill_string(self) -> None:
        ...
        assert t['name'][idx] is None
        assert isinstance(
            t['name'].dtype, np.dtypes.StringDType)
    ...
    def test_left_join_c_vs_python_same_results(self) -> None:
        ...
        c_rows = sorted(
            [(t_c['k'][i], t_c['lv'][i]) for i in range(len(t_c))]
        )
        py_rows = sorted(
            [(t_py['k'][i], t_py['lv'][i])
...
class TestCrossJoin:
    ...
    def test_cross_join_exact_pairs(self) -> None:
        ...
        for i in range(len(t)):
            ...
            pairs.add((
                row_vals[lk_idx], row_vals[lv_idx],
                row_vals[rk_idx], row_vals[rv_idx],
            ))
...
class TestChunking:
    def test_chunks_basic(self) -> None:
        ...
        t = Tafra({
            'x': np.arange(10), 'y': np.arange(10.0)})
        parts = t.chunks(3)
        assert len(parts) == 3
    ...
    def test_partition_basic(self) -> None:
        ...
        assert keys == [(1,), (2,), (3,)]
        np.testing.assert_array_equal(
            parts[0][1]['v'], np.array([10., 30.]))
        np.testing.assert_array_equal(
            parts[1][1]['v'], np.array([20., 40.]))
...
try:
    from tafra._accel import (
        composite_key, group_indices, encode_strings)
    _HAS_ACCEL = True
except ImportError:
    _HAS_ACCEL = False
...
@pytest.mark.skipif(
    not _HAS_ACCEL, reason='C extension not available')
...
class TestGroupIndices:
    ...
    def test_first_seen_order(self) -> None:
        fs, gl, ng = group_indices(
            np.array([3, 1, 3, 2, 1], dtype=np.int64))
        assert ng == 3
