...
def print_header():
    ...
    print(f"  {'Benchmark':<35s} {accel_label:>9s} {'Tafra':>9s} {'pandas':>9s} {'polars':>9s}")
    print(f"  {'-' * 35} {'-' * 9} {'-' * 9} {'-' * 9} {'-' * 9}")
...
def bench_row_mapping():
    ...
    for n_rows, label in [
        ...
        t_pandas = median_of(lambda: [row_fn(r.a, r.b, r.c) for r in df.itertuples()], n=n_rep)
        t_polars = None
        if HAS_POLARS:
            plf = pl.DataFrame(tf.data)
            t_polars = median_of(
                lambda: plf.with_columns(
                    pl.struct(["a", "b", "c"])
                    .map_elements(lambda s: row_fn(s["a"], s["b"], s["c"]), return_dtype=pl.Float64)
                    .alias("result")
                ),
                n=n_rep,
            )
...
def bench_vectorized_expr():
    ...
    for n_rows, label in [
        ...
        t_tafra = median_of(
            lambda: np.sqrt(tf["a"] ** 2 + tf["b"] ** 2) + np.log1p(np.abs(tf["c"]))
        )
        # pandas: same numpy ops on .values, or pandas API
        t_pandas = median_of(
            lambda: np.sqrt(df["a"] ** 2 + df["b"] ** 2) + np.log1p(np.abs(df["c"]))
        )
        t_polars = None
        if HAS_POLARS:
            plf = pl.DataFrame(tf.data)
            t_polars = median_of(
                lambda: plf.with_columns(
                    (
                        (pl.col("a") ** 2 + pl.col("b") ** 2).sqrt() + (pl.col("c").abs() + 1).log()
                    ).alias("result")
                )
            )
        print_row(label, t_tafra, t_tafra, t_pandas, t_polars)
...
def bench_groupby():
    ...
    for n_rows, label in [(100_000, "100k, 2 col, ~300 grp"), (1_000_000, "1M, 2 col, ~300 grp")]:
        ...
        df = pd.DataFrame(tf.data)
        t_accel, t_pure = _tafra_time(
            lambda: tf.group_by(["g1", "g2"], {"mean_val": (np.mean, "value")})
        )
        t_pandas = median_of(lambda: df.groupby(["g1", "g2"])["value"].agg("mean").reset_index())
        t_polars = None
        if HAS_POLARS:
            plf = pl.DataFrame(tf.data)
            t_polars = median_of(
                lambda: plf.group_by("g1", "g2").agg(pl.col("value").mean().alias("mean_val"))
            )
        print_row(label, t_accel, t_pure, t_pandas, t_polars)
...
def bench_transform():
    ...
    for n_rows, n_groups, label in [
        ...
        t_accel, t_pure = _tafra_time(
            lambda: tf.transform(["group"], {"mean_val": (np.mean, "value")})
        )
        t_pandas = median_of(
        ...
        if HAS_POLARS:
            plf = pl.DataFrame(tf.data)
            t_polars = median_of(
                lambda: plf.with_columns(pl.col("value").mean().over("group").alias("mean_val"))
            )
        print_row(label, t_accel, t_pure, t_pandas, t_polars)
...
def bench_joins():
    ...
    for n, label in [
        ...
        t_accel, t_pure = _tafra_time(lambda: left_tf.inner_join(right_tf, [("key", "key", "==")]))
        t_pandas = median_of(lambda: pd.merge(left_df, right_df, on="key", how="inner"))
        t_polars = None
        if HAS_POLARS:
    ...
    for n, label in [(1000, "1k x 1k"), (5000, "5k x 5k"), (50000, "50k x 50k")]:
        ...
        t_accel, t_pure = _tafra_time(lambda: left_tf.left_join(right_tf, [("key", "key", "==")]))
        t_pandas = median_of(lambda: pd.merge(left_df, right_df, on="key", how="left"))
        t_polars = None
        if HAS_POLARS:
            left_pl = pl.DataFrame(left_tf.data)
            right_pl = pl.DataFrame(right_tf.data)
...
def bench_numba():
    ...
    for n_rows, label in [
        ...
        if HAS_POLARS:
            plf = pl.DataFrame(tf.data)
            t_polars = median_of(
                lambda: numba_fn(plf["a"].to_numpy(), plf["b"].to_numpy(), plf["c"].to_numpy())
            )
